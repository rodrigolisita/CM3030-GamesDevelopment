...
public class GameManager : MonoBehaviour
{
    ...
    void Awake()
    {
        ...
        // If Instance == this, it's the persistent GameManager.
        // OnSceneLoaded will handle UI re-linking and updates.
    }

    void OnDestroy()
    {
        // Unsubscribe from the event when the GameManager is truly destroyed
    ...
    {
        Debug.Log("GameManager OnSceneLoaded: Scene '" + scene.name + "' loaded. Re-finding UI elements and updating display.");
    ...
    void FindUIElements()
    {
        Debug.Log("GameManager FindUIElements: Attempting to find UI elements by name.");
        ...
        else Debug.LogError("GameManager FindUIElements: Could not find ScoreText GameObject named: '" + scoreTextName + "'. Ensure it exists in the scene and the name matches.", this.gameObject);
        ...
        else Debug.LogError("GameManager FindUIElements: Could not find LivesText GameObject named: '" + livesTextName + "'.", this.gameObject);
        ...
        else Debug.LogError("GameManager FindUIElements: Could not find GameOverText GameObject named: '" + gameOverTextName + "'.", this.gameObject);
        ...
        else Debug.LogError("GameManager FindUIElements: Could not find RestartButton GameObject named: '" + restartButtonName + "'.", this.gameObject);
        ...
        if (restartButton != null)
        {
            ...
            Debug.Log("GameManager FindUIElements: RestartButton listener re-assigned.");
        }
    }
    ...
    void ResetInternalGameState()
    {
        ...
        Debug.Log("GameManager ResetInternalGameState: Game state reset. isGameActive: " + isGameActive + ", Score: " + score + ", Lives: " + currentLives);
    ...
    {
        Debug.Log("GameManager UpdateAllUIDisplays: Updating UI. Current isGameActive: " + isGameActive);

        // Update score display
        if (scoreText != null) 
        {
            scoreText.text = "Score: " + score; 
            Debug.Log("GameManager UpdateAllUIDisplays: Score UI updated to: " + scoreText.text);
        }
        ...
        if (livesText != null) 
        {
            ...
            Debug.Log("GameManager UpdateAllUIDisplays: Lives UI updated via UpdateLivesDisplay(). Current lives on UI: " + (livesText.text));
        }
        ...
        {
            Debug.Log("GameManager UpdateAllUIDisplays: Setting GameOverText active state to: " + showGameOverUI + " (because isGameActive is " + isGameActive + ")");
        ...
        if (restartButton != null) 
        {
            Debug.Log("GameManager UpdateAllUIDisplays: Setting RestartButton active state to: " + showGameOverUI + " (because isGameActive is " + isGameActive + ")");
    ...
    // For a persistent GameManager, this will only run when it's first created.
    void Start()
    {
        Debug.Log("GameManager Start() called (should only be once for persistent instance).");
    }
    ...
    {
        Debug.Log("GameManager: RestartGame() called. Resetting internal state and reloading scene.");
